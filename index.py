...
cred_path = os.path.join(os.getcwd(), "plant-disease-detection-2e61f-firebase-adminsdk-fbsvc-f2ea4add9d.json")
        ...
        if not os.path.exists(cred_path):
            ...
            try:
                ...
                with open(cred_path, 'r') as cred_file:
                    ...
                    if missing_fields:
                        st.error(f"Missing required fields in credentials: {', '.join(missing_fields)}")
                        logging.error(f"Missing credential fields: {missing_fields}")
                    else:
                ...
                logging.error(f"Firebase init error: {str(e)}")
    except Exception as e:
        st.error(f"Error accessing credentials file: {str(e)}")
        logging.error(f"File access error: {str(e)}")

# -----------------------
# 2. DEFINE LOGIN/SIGNUP
# -----------------------
# Update login function with JWT handling
def firebase_login(email: str, password: str):
    """Enhanced login function with JWT validation"""
    try:
        ...
        try:
            ...
            # Create a custom token (for testing only)
            custom_token = auth.create_custom_token(user_record.uid)
            
            logging.info(f"Login successful for user: {email}")
            
            return {
                "idToken": custom_token.decode(),
            ...
            logging.error(f"Invalid token for user: {email}")
...
def firebase_signup(email: str, password: str):
    ...
    try:
        ...
        return {
            "idToken": "MOCK_TOKEN",  # Using a mock token for demo
            "email": user_record.email,
            "uid": user_record.uid
        }
    except firebase_admin._auth_utils.EmailAlreadyExistsError:
        logging.warning(f"Signup attempt failed - email exists: {email}")
        return {"error": "Email already exists. Please login instead."}
    except ValueError as e:
        logging.warning(f"Invalid input for signup: {str(e)}")
        return {"error": str(e)}
    except Exception as e:
...
def model_prediction(image_data):
    ...
    try:
        ...
        idx = np.argmax(preds[0])  # Get the index of highest probability
        conf = float(preds[0][idx])  # Get the confidence score
    ...
    if test_image:
        if st.button("🔍 Analyze Image"):
            with st.spinner("🔄 Analyzing your image..."):
                disease_name, confidence = model_prediction(test_image)
